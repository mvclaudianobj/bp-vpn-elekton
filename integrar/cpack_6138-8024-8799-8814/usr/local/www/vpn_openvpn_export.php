...
if (!function_exists('bluepex_is_entraid_server')) {
	function bluepex_is_entraid_server($server) {
		...
		$authserversRaw = $config['system']['authserver'];
		$authservers = array();

		/*
		 * pfSense pode carregar authserver em dois formatos:
		 * 1) Lista: [0 => ['name'=>..., 'type'=>...], ...]
		 * 2) Único objeto: ['name'=>..., 'type'=>...]
		 */
		if (is_array($authserversRaw) && isset($authserversRaw['name'])) {
			$authservers[] = $authserversRaw;
		} elseif (is_array($authserversRaw)) {
			foreach ($authserversRaw as $entry) {
				if (is_array($entry)) {
					$authservers[] = $entry;
				}
			}
		...
		$isEntraType = function($typeValue) {
			return !empty($typeValue) && strtolower(trim((string)$typeValue)) === 'entraid';
		};

		$authServerCandidates = array();
		$possibleFields = array('authserver', 'auth_server', 'authmode', 'auth_mode');
		foreach ($possibleFields as $f) {
			if (!empty($server[$f])) {
				$val = trim((string)$server[$f]);
				if ($val !== '' && strtolower($val) !== 'local database') {
					$authServerCandidates[] = $val;
				}
			}
		}

		$authServerCandidates = array_values(array_unique($authServerCandidates));

		if (empty($authServerCandidates)) {
			return false;
		}

		if (function_exists('auth_get_authserver')) {
			foreach ($authServerCandidates as $candidate) {
		...
		if (@file_exists('/etc/entraid_auth_enable')) {
			$activeEntraAuth = trim((string)@file_get_contents('/etc/entraid_auth_enable'));
			if ($activeEntraAuth !== '' && in_array($activeEntraAuth, $authServerCandidates, true)) {
				return true;
			}
		}
		...
		foreach ($authservers as $as) {
			...
			$type = isset($as['type']) ? trim((string)$as['type']) : '';
			if ($name !== '' && in_array($name, $authServerCandidates, true) && $isEntraType($type)) {
				return true;
			}
		}
...
if (!function_exists('bluepex_build_azure_tags')) {
	function bluepex_build_azure_tags() {
		...
		$clientId = trim((string)($env['CLIENT_ID'] ?? ''));
		$tenantId = trim((string)($env['TENANT_ID'] ?? ''));
		if ($tenantId === '' && !empty($env['AUTHORITY'])) {
		...
		$serverApi = trim((string)($env['SERVER_API'] ?? ''));
		if ($serverApi === '' && !empty($env['FASTAPI_SERVER'])) {
			$serverApi = rtrim(trim((string)$env['FASTAPI_SERVER']), '/') . '/publish';
		}
		if ($serverApi === '') {
...
if (!function_exists('bluepex_append_azure_tags_to_ovpn')) {
	function bluepex_append_azure_tags_to_ovpn($ovpn, $serverCfg) {
		if (!is_string($ovpn) || $ovpn === '') {
			return $ovpn;
		}
		if (!bluepex_is_entraid_server($serverCfg)) {
			return $ovpn;
		}
...
if (!function_exists('bluepex_append_azure_tags_to_zip')) {
	function bluepex_append_azure_tags_to_zip($zipPath, $serverCfg) {
		...
		if (class_exists('ZipArchive')) {
			...
			if ($openResult === true) {
				for ($i = 0; $i < $zip->numFiles; $i++) {
					...
					if (!is_string($entryName) || substr($entryName, -1) === '/') {
						continue;
					}
					...
					$looksLikeOvpnByName = (substr($lowerName, -5) === '.ovpn' || substr($lowerName, -5) === '.conf');
					$looksLikeOvpnByContent = (
						strpos($content, "\nclient\n") !== false ||
						strpos($content, "\nremote ") !== false ||
					...
					if (!$looksLikeOvpnByName && !$looksLikeOvpnByContent) {
						continue;
					}

					$updated = bluepex_append_azure_tags_to_ovpn($content, $serverCfg);
					if ($updated !== $content) {
						if ($zip->setFromName($entryName, $updated) !== false) {
							$patched = true;
						}
					}
				}
		...
		$tmpDir = sys_get_temp_dir() . '/bluepex_zip_' . uniqid('', true);
		@mkdir($tmpDir, 0700, true);

		$unzipCmd = '/usr/bin/unzip -oq ' . escapeshellarg($zipPath) . ' -d ' . escapeshellarg($tmpDir);
		exec($unzipCmd, $out1, $rc1);
		if ($rc1 !== 0) {
			exec('/bin/rm -rf ' . escapeshellarg($tmpDir));
			return $zipPath;
		}
		...
		foreach ($rii as $fileInfo) {
			...
			$looksLikeOvpnByName = (substr($name, -5) === '.ovpn' || substr($name, -5) === '.conf');
			$looksLikeOvpnByContent = (
				strpos($content, "\nclient\n") !== false ||
				strpos($content, "\nremote ") !== false ||
				strpos($content, "auth-user-pass") !== false ||
		...
		if ($changed) {
			...
			if ($rc2 !== 0 && @is_executable('/usr/local/bin/7z')) {
				$zipCmd = '/usr/local/bin/7z a -tzip ' . escapeshellarg($zipOut) . ' .';
				exec($zipCmd, $out3, $rc3);
				$rc2 = $rc3;
			}

			chdir($cwd);
			if ($rc2 === 0 && file_exists($zipOut)) {
				@rename($zipOut, $zipPath);
			}
		}
