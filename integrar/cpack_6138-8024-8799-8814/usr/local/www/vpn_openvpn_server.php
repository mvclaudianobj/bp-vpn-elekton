...
require_once("openvpn_control.inc");
...
if ($_POST['ajax_table_openvpn']) {
	$returnTable = "";

	$returnTable .= "<div class=\"panel panel-default\">";
	$returnTable .= "<div class=\"panel-heading\"><h2 class=\"panel-title\">" . gettext('OpenVPN Servers') . "</h2></div>";
	$returnTable .= "<div class=\"panel-body table-responsive\">";
	$returnTable .= "<table class=\"table table-striped table-hover table-condensed sortable-theme-bootstrap table-rowdblclickedit\" data-sortable>";
	$returnTable .= "<thead>";
	$returnTable .= "<tr>";
	$returnTable .= "<th>" . gettext("Interface") . "</th>";
	$returnTable .= "<th data-sortable-type=\"alpha\">" . gettext("Protocol / Port") . "</th>";
	$returnTable .= "<th>" . gettext("Tunnel Network") . "</th>";
	$returnTable .= "<th>" . gettext("Crypto") . "</th>";
	$returnTable .= "<th>" . gettext("Description") . "</th>";
	$returnTable .= "<th>" . gettext("Actions") . "</th>";
	$returnTable .= "</tr>";
	$returnTable .= "</thead>";
	$returnTable .= "<tbody>";
	$i = 0;
	...
	foreach ($a_server as $server) {
		$interfaceNow = trim(convert_openvpn_interface_to_friendly_descr($server['interface'])) ;
		$returnTable .= "<tr";

		if (isset($server['disable'])) {
			$returnTable .= ' class="disabled">';
		} else {
			$returnTable .= '>';
		}

		$returnTable .= "<td>{$interfaceNow} (" . strtoupper($server['interface']) . ")</td>";
		$returnTable .= "<td data-value='" . htmlspecialchars($server['local_port']) . '-' . htmlspecialchars($server['protocol']) . "'>";
		$returnTable .= htmlspecialchars($server['protocol']) . "/" .  htmlspecialchars($server['local_port']);
		$returnTable .= "</td>";
		$returnTable .= "<td>";
		$returnTable .= htmlspecialchars($server['tunnel_network']) . "<br />";
		$returnTable .= htmlspecialchars($server['tunnel_networkv6']);
		$returnTable .= "</td>";
		$returnTable .= "<td>";
		$returnTable .= sprintf('Crypto: %1$s/%2$s', $server['crypto'], $server['digest']);

		if (is_numeric($server['dh_length'])) {
			$returnTable .= sprintf("<br/>D-H Params: %d bits", $server['dh_length']);
		} elseif ($server['dh_length'] == "none") {
			$returnTable .= "<br />D-H Disabled, using ECDH Only";
		}

		$returnTable .= "</td>";
		$returnTable .= "<td>";
		$returnTable .= htmlspecialchars(sprintf('%1$s (%2$s)', $server['description'], $server['dev_mode']));
		$returnTable .= "</td>";
		$returnTable .= "<td>";
		$returnTable .= " <a href='#' class=\"fa fa-pencil\" title='" . gettext('Edit Server') . "' onclick=\"operation_vpn_server('edit',{$i})\"></a>";
		$returnTable .= " <a href='#' class=\"fa fa-clone\" title='" . gettext("Copy Server") . "' onclick=\"operation_vpn_server('dup',{$i})\"></a>";
		$returnTable .= " <a href='#' class=\"fa fa-trash\" title='" . gettext('Delete Server') . "' onclick=\"operation_vpn_server('del',{$i})\"></a>";
		$returnTable .= "</td>";
		$returnTable .= "</tr>";
		$i++;
	}

	$returnTable .= "</tbody></table></div></div>";
	$returnTable .= "<script src='/js/pfSense.js?v=".filemtime('/usr/local/www/js/pfSense.js')."'></script>";
	$returnTable .= "<script src='/js/pfSenseHelpers.js?v=".filemtime('/usr/local/www/js/pfSenseHelpers.js')."'></script>";
...
if ($act == "del") {
	...
	} elseif (!$user_can_edit_advanced &&
	    !empty($a_server[$id]['custom_options'])) {
		$input_errors[] = gettext("This user does not have sufficient privileges to delete an instance with Advanced options set.");
	} else {
		bp_write_report_db("report_0008_openvpn_server_del", "{$a_server[$id]['description']}|" . convert_friendly_interface_to_friendly_descr($a_server[$id]['interface']));
	...
	$vpnid_server = $a_server[$id]['vpnid'];

	if (isset($opvc['rules']['rule']) &&
	    !empty($opvc['rules']['rule'])) {
		foreach ($opvc['rules']['rule'] as $key => $rule_instance_openvpn) {
			if (!isset($rule_instance_openvpn['vpninstances']) ||
			    empty($rule_instance_openvpn['vpninstances'])) {
				continue;
			}

			$position = (int) $key;

			$opvc['rules']['rule'][$position]['vpninstances'] = implode("|", array_diff(explode("|", $rule_instance_openvpn['vpninstances']), ["{$vpnid_server}"]));
		}
	}
	...
	if (!empty($wc_msg)) {
		...
		if (is_string($opvc)) {
			unset($opvc);
			$opvc = &$config['installedpackages']['openvpncontrol'];
			$opvc = ['cleardata' => []];
		}

		write_config($wc_msg);
		$savemsg = gettext("Server successfully deleted. " . implode(",", $test));
	}
}

if ($act == "new") {
	$pconfig['ncp_enable'] = "enabled";
	$pconfig['ncp-ciphers'] = "AES-256-CBC";
...
if (($act == "edit") ||
	...
	if (isset($id) &&
		...
		$pconfig['mult-factor'] = $a_server[$id]['mult-factor'];
		...
		$pconfig['client_auth_nocache'] = $a_server[$id]['client_auth_nocache'];
...
$confirm_dup_act = false;
$confirm_dup_act_text = "";

if ($act == "dup") {
	$confirm_dup_act = true;
	$confirm_dup_act_text = "{$a_server[$id]['description']}|" . convert_friendly_interface_to_friendly_descr($a_server[$id]['interface']) . "|";
	$act = "new";
	$pconfig['local_port'] = openvpn_port_next('UDP');
	$vpnid = 0;
	unset($id);
...
if ($_POST['save']) {
	...
	if (is_ipaddrv4($iv_ip) &&
	    (stristr($pconfig['protocol'], "6") !== false)) {
	...
	if (empty($pconfig['authmode']) &&
	    (($pconfig['mode'] == "server_user") ||
	    ($pconfig['mode'] == "server_tls_user"))) {
	...
	if ($pconfig['dns_server_enable']) {
		if (!empty($pconfig['dns_server1']) &&
		    !is_ipaddr(trim($pconfig['dns_server1']))) {
			$input_errors[] = gettext("The field 'DNS Server #1' must contain a valid IPv4 or IPv6 address");
		}

		if (!empty($pconfig['dns_server2']) &&
		    !is_ipaddr(trim($pconfig['dns_server2']))) {
			$input_errors[] = gettext("The field 'DNS Server #2' must contain a valid IPv4 or IPv6 address");
		}

		if (!empty($pconfig['dns_server3']) &&
		    !is_ipaddr(trim($pconfig['dns_server3']))) {
			$input_errors[] = gettext("The field 'DNS Server #3' must contain a valid IPv4 or IPv6 address");
		}

		if (!empty($pconfig['dns_server4']) &&
		    !is_ipaddr(trim($pconfig['dns_server4']))) {
			$input_errors[] = gettext("The field 'DNS Server #4' must contain a valid IPv4 or IPv6 address");
		}
	}

	if ($pconfig['ntp_server_enable']) {
		if (!empty($pconfig['ntp_server1']) &&
		    !is_ipaddr(trim($pconfig['ntp_server1']))) {
			$input_errors[] = gettext("The field 'NTP Server #1' must contain a valid IP address");
		}

		if (!empty($pconfig['ntp_server2']) &&
		    !is_ipaddr(trim($pconfig['ntp_server2']))) {
			$input_errors[] = gettext("The field 'NTP Server #2' must contain a valid IP address");
		}

		if (!empty($pconfig['ntp_server3']) &&
		    !is_ipaddr(trim($pconfig['ntp_server3']))) {
			$input_errors[] = gettext("The field 'NTP Server #3' must contain a valid IP address");
		}

		if (!empty($pconfig['ntp_server4']) &&
	...
	if ($pconfig['netbios_enable']) {
		if ($pconfig['wins_server_enable']) {
			if (!empty($pconfig['wins_server1']) &&
			    !is_ipaddr(trim($pconfig['wins_server1']))) {
				$input_errors[] = gettext("The field 'WINS Server #1' must contain a valid IP address");
			}

			if (!empty($pconfig['wins_server2']) &&
			    !is_ipaddr(trim($pconfig['wins_server2']))) {
				$input_errors[] = gettext("The field 'WINS Server #2' must contain a valid IP address");
			}
		}

		if ($pconfig['nbdd_server_enable']) {
			if (!empty($pconfig['nbdd_server1']) &&
			    !is_ipaddr(trim($pconfig['nbdd_server1']))) {
				$input_errors[] = gettext("The field 'NetBIOS Data Distribution Server #1' must contain a valid IP address");
			}
		}
	}

	if ($pconfig['maxclients'] &&
	    !is_numericint($pconfig['maxclients'])) {
	...
	if ($pconfig['mode'] != "p2p_shared_key") {
		...
		if (!empty($pconfig['dh_length']) &&
		    !in_array($pconfig['dh_length'], array_keys($openvpn_dh_lengths))) {
		...
		if (!empty($pconfig['ecdh_curve']) &&
		    !openvpn_validate_curve($pconfig['ecdh_curve'])) {
			$input_errors[] = gettext("The specified ECDH Curve is invalid.");
		}

		if (($pconfig['ncp_enable'] != "disabled") &&
		    !empty($pconfig['ncp-ciphers']) &&
		    is_array($pconfig['ncp-ciphers'])) {
	...
	if ($pconfig['dev_mode'] != "tap") {
		...
		if (($pconfig['serverbridge_dhcp'] &&
		    ...
		    ((empty($pconfig['serverbridge_interface'])) ||
		    (strcmp($pconfig['serverbridge_interface'], "none") == 0))) {
			$input_errors[] = gettext("Bridge Route Gateway requires a valid Bridge Interface.");
		}

		if (($pconfig['serverbridge_dhcp_start'] &&
		...
		if (($pconfig['serverbridge_dhcp_start'] &&
		    !is_ipaddrv4($pconfig['serverbridge_dhcp_start']))) {
			$input_errors[] = gettext("Server Bridge DHCP Start must be an IPv4 address.");
		}

		if (($pconfig['serverbridge_dhcp_end'] &&
		    !is_ipaddrv4($pconfig['serverbridge_dhcp_end']))) {
	...
	if ($pconfig['udp_fast_io'] &&
	    (strtolower(substr($pconfig['protocol'], 0, 3)) != "udp")) {
		unset($pconfig['udp_fast_io']);
	}

	if (!empty($pconfig['sndrcvbuf']) &&
	    !array_key_exists($pconfig['sndrcvbuf'], openvpn_get_buffer_values())) {
		$input_errors[] = gettext("The supplied Send/Receive Buffer size is invalid.");
	}

	if (!empty($pconfig['ping_method']) &&
	    !array_key_exists($pconfig['ping_method'], $openvpn_ping_method)) {
		$input_errors[] = gettext("The supplied Ping Method is invalid.");
	}

	if (!empty($pconfig['ping_action']) &&
	    !array_key_exists($pconfig['ping_action'], $openvpn_ping_action)) {
		$input_errors[] = gettext("The supplied Ping Action is invalid.");
	}

	if (!empty($pconfig['keepalive_interval']) &&
	    !is_numericint($pconfig['keepalive_interval'])) {
		$input_errors[] = gettext("The supplied Keepalive Interval value is invalid.");
	}

	if (!empty($pconfig['keepalive_timeout']) &&
	    !is_numericint($pconfig['keepalive_timeout'])) {
		$input_errors[] = gettext("The supplied Keepalive Timeout value is invalid.");
	}

	if (!empty($pconfig['ping_seconds']) &&
	    !is_numericint($pconfig['ping_seconds'])) {
		$input_errors[] = gettext("The supplied Ping Seconds value is invalid.");
	}

	if (!empty($pconfig['ping_action_seconds']) &&
	    !is_numericint($pconfig['ping_action_seconds'])) {
		$input_errors[] = gettext("The supplied Ping Restart or Exit Seconds value is invalid.");
	}

	if (!empty($pconfig['inactive_seconds']) &&
	    !is_numericint($pconfig['inactive_seconds'])) {
	...
	if (!$input_errors) {
		...
		if ($_POST['disable'] == "yes" &&
		    !isset($a_server[$id]['disable'])) {
			$text_status = "report_0008_openvpn_server_disabled";
		} elseif (!isset($_POST['disable']) &&
		    isset($a_server[$id]['disable'])) {
			$text_status = "report_0008_openvpn_server_enabled";
		}

		if (!empty($text_status)) {
			bp_write_report_db("{$text_status}", "{$a_server[$id]['description']}|" . convert_friendly_interface_to_friendly_descr($a_server[$id]['interface']));
		}
		...
		$server['mult-factor'] = $pconfig['mult-factor'];
		...
		$server['client_auth_nocache'] = $pconfig['client_auth_nocache'];
		...
		if (isset($pconfig['isdup'])) {
			$log_text = "dup";
			$extra_text = $pconfig['isdup'];
		}

		if (!empty($log_text)) {
			bp_write_report_db("report_0008_openvpn_server_{$log_text}", "{$extra_text}{$server['description']}|" . convert_friendly_interface_to_friendly_descr($server['interface']));
		}
...
<div class="infoblock">
	<div class="alert alert-info warning clearfix" role="alert">
		<div class="pull-left">
			<?=gettext("O modelo de criptografia \"AES-128-GCM\" do OpenVPN foi descontinuada nas versões mais recentes do recurso, esteja ciente que conexões que utilizavam desta cifrá não iram funcionar.")?>
			<br>
			<?=gettext("Em caso de dúvidas, entre em contato com o suporte técnico.")?>
		</div>
	</div>
</div>
...
function form_generate_cert(&$section)
{
	...
	$section->addInput(new Form_Input(
		'dn_state',
		gettext('State or Province'),
		'text',
		$pconfig['dn_state']
	))->setHelp(gettext('e. g. São Paulo'));
...
if ($act == "new" ||
	...
	if (empty($authmodes[0]) &&
	    !empty(key($auth_servers))) {
		$authmodes[0] = key($auth_servers);
	}

	foreach ($auth_servers as $auth_server_key => $auth_server) {
		/*if (isset($auth_server['type']) &&
		    $auth_server['type'] == 'entraid') {
			continue;
		}*/
	...
	$buttonGenerate = new Form_Button(
		'generatecertca',
		gettext('Generate New Certificate')
	);
	...
	if (count($a_ca)) {
		...
		$groupCertAuth = new Form_Group('Peer Certificate Authority');
		...
		$section->addInput($buttonGenerate);
	...
	if (count($a_crl)) {
		...
		if (count($a_ca)) {
			$section->addInput(new Form_Button(
				'generatecert',
				gettext('Generate Certificate')
			));
		} else {
			$section->addInput(new Form_StaticText(
				gettext('*Peer Certificate Revocation list'),
				sprintf(gettext('No Certificate Revocation Lists defined. One may be created here: %s'), '<a href="system_camanager.php">' . gettext('System &gt; Cert. Manager') . '</a>')
			));
		}
	}
	...
	))->setHelp(gettext('Compress tunnel packets using the LZO algorithm. %1$s' .
	...
	$section = new Form_Section('Mult-Factor Authentication');
	$section->addClass('advanced');

	$section->addInput(new Form_Checkbox(
		'mult-factor',
		'2FA with Google Authenticator',
		'Allow mult-factor autentication for login OpenVPN.',
		$pconfig['mult-factor']
	));
	...
	$section = new Form_Section(gettext("Ping settings"));
	...
	$section->addInput(new Form_Input(
		...
		['min' => '0']
	))->setHelp(gettext('Causes OpenVPN to exit after n seconds of ' .
	...
	$section->addInput(new Form_Select(
		...
		$pconfig['ping_method'],
		$openvpn_ping_method
	))->setHelp(gettext('keepalive helper uses interval and timeout parameters ' .
	...
	$section->addInput(new Form_Input(
		...
		'Interval',
		'number',
		$pconfig['keepalive_interval'] ?: $openvpn_default_keepalive_interval,
		['min' => '0']
	));
	...
	$section->addInput(new Form_Input(
		...
		'Timeout',
		'number',
		$pconfig['keepalive_timeout'] ?: $openvpn_default_keepalive_timeout,
		['min' => '0']
	));
	...
	$section->addInput(new Form_Input(
		...
		'number',
		$pconfig['ping_action_seconds'] ?: $openvpn_default_keepalive_timeout,
		['min' => '0']
	));
	...
	$section->addInput(new Form_Checkbox(
		'client_auth_nocache',
		'Not cache passwords',
		'If enabled, does not cache passwords.',
		$pconfig['client_auth_nocache']
	));
...
<div id="ajax_table_openvpn" style="margin-top:80px;">
<?php print_info_box(gettext("Please a moment, loading information tables..."), 'info', false); ?>
</div>
...
<form id="submit_changes" method="POST" style="display:none;">
...
events.push(function() {
	...
	function error_Message(msg) {
		...
		html += "	<?php echo '<p>' . gettext('The following input errors were detected:') . '</p>'; ?>";
		html += "	<ul>";
		html += "		<li> " + msg + " </li>";
		html += "	</ul>";
		html += "</div>";
	...
	function validation_Form(certName, country, state, city, organization, email, commonname) {
		 if (!certName) {
			error_Message("<?=gettext('The fild Certificate Name is required.');?>");
			return false;
		} else if (!country) {
			error_Message("<?=gettext('The fild Country is required.');?>");
			return false;
		} else if (!state) {
			error_Message("<?=gettext('The fild State or Province is required.');?>");
			return false;
		} else if (!city) {
			error_Message("<?=gettext('The fild City is required.');?>");
			return false;
		} else if (!organization) {
			error_Message("<?=gettext('The fild Organization is required.');?>");
			return false;
		} else if (!email) {
			error_Message("<?=gettext('The fild Email address is required.');?>");
			return false;
		} else if (!commonname) {
			error_Message("<?=gettext(' fild Common name is required.');?>");
			return false;
		} else {
			return true;
		}
	...
	function save_certificateCA() {
		...
		var save = "<?=gettext('Save')?>";

		if(!validation_Form(certName, country, state, city, organization, email, commonname)) {
			return false;
		}

		var pars = 'descr=' + certName + '&method=internal&caref=&keylen=' + keylen + '&digest_alg=' + digest_alg + '&keytype=' + keytype + '&ecname=' + ecname + '&lifetime=' + lifetime + '&dn_country=' + country + '&dn_state=' + state + '&dn_city=' + city + '&dn_organization=' + organization + '&dn_email=' + email + '&dn_commonname=' + commonname + '&save=' + save;
	...
	function make_certificate(carefID) {
		...
		$.ajax(
			url,
			{
				...
				success: function(response) {
					make_certRevogation(certName, carefID, response);
				}
			});
	}
	...
	function make_certRevogation(certName, caref, certrefID) {
		if(!caref) {
			error_Message("Error to create Revogation Certificate.");
			return;
		}

		var url = "/system_crlmanager.php";
		var save = "<?=gettext('Save')?>";
		var CertNameRev = certName + '_Rev';
		var pars = 'method=internal&descr=' + CertNameRev + '&caref=' + caref + '&crltext=&lifetime=9999&serial=&save=' + save;
		...
		$.ajax(
			url,
			{
				type: 'post',
				data: pars,
				error: generatecertRevogation_failure,
				success: function(response) {
					// Insert option in Peer Certificate Authority
					var crlrefID = response;
					setProperties(certName, caref, certrefID, crlrefID);
					...
					var CarefInput = "<input name=\"caref\" type=\"hidden\" value="+caref+"  />";
					$("#iform").append(CarefInput);

					alert("<?=gettext('Success to Certificate Generate!');?>");
					location.reload();
					exit;
				}
			});
	...
	function generatecertRevogation_failure() {
		error_Message("Sorry, we could not create your certificate Revogation at this time.");
		return;
	}

	function setProperties(CertName, caref, certrefID, crlrefID) {
		var CertDesc = CertName + "(CA: " + CertName + ")";

		$('#caref').append(new Option(CertName, caref));
		$("#caref").find("option:contains("+CertName+")").each(function(){
			if ($(this).val() == escape(caref)) {
				$(this).attr("selected","selected");
			}
		 });

		$('#crlref').append(new Option(CertDesc, escape(crlrefID)));
		$("#crlref").find("option:contains("+CertDesc+")").each(function(){
			if ($(this).val() == escape(crlrefID)) {
				$(this).attr("selected","selected");
			}
		 });

		$('#certref').append(new Option(CertDesc, escape(certrefID)));
		$("#certref").find("option:contains("+CertDesc+")").each(function(){
			if( $(this).val() == escape(certrefID) ) {
				$(this).attr("selected","selected");
			}
		});
	}
	...
	$('#generatecertca').on('click', function(e){
	 ...
	 // Automation to generate certificate
	$('#generatecert').on('click', function(e){
		e.preventDefault();
		generate_certificateRevogation();
	});
...
?>
<script>
function ajax_table_openvpn() {
	$.ajax({
		url: '/vpn_openvpn_server.php',
		method: 'POST',
		data: {
			ajax_table_openvpn: 'true',
		},
		success: function(data) {
			$("#ajax_table_openvpn").html(data);
		}
	});
}

setInterval(ajax_table_openvpn, 15000);

setTimeout(() => {
	ajax_table_openvpn();
}, 100);

function operation_vpn_server(action, idvpn) {
	$("#id").val(idvpn);
	$("#act").val(action);
