...
- **`checkForUpdates` aguarda evento real**: Corrigido retorno falso imediato que reportava "versão mais recente" sem verificar o GitHub. Agora a função aguarda o evento `update-available` ou `update-not-available` antes de responder ao renderer.
- **`openExternal` exposto no `preload.js`**: Permite que o renderer abra URLs externas com segurança via `shell.openExternal`.
- **Versão corrigida**: Campo `version` corrigido de `1.0.5` para `0.1.5` para compatibilidade com auto-update semântico.

### 🔧 Correções de Bugs (acumulado)

- **Correção de Salvamento de Credenciais**: Corrigido bug onde senhas não eram salvas ao marcar "Lembrar credenciais".
- **Correção de Ícones no App Packaged**: Implementado protocolo customizado `local-resource://` para servir ícones corretamente no app empacotado.
- **Correção de HTML Duplicado**: Removidas seções HTML duplicadas no `index.html`.
- **Correção de Event Listeners Duplicados**: Removida chamada duplicada de `setupEventListeners()`.
- **Persistência de senha corrigida**: Ajustado uso de chaves de estado (`selectedProfileId`/`selectedProfileType`).
- **Criptografia de credenciais corrigida**: Substituídas APIs inválidas por `createCipheriv`/`createDecipheriv` com AES-256-GCM.
- **Estado do túnel VPN validado na inicialização**: Valida PID + processo OpenVPN real para evitar UI conectada com túnel caído.
